...
public class MyImageObj extends JLabel {
	...
	private TextSetWindow tWindow = null;
	private FontGuessWindow guessWindow;
	...
	public void resetImage() {
		...
		//build a new fontGuessWindow
		guessWindow = new FontGuessWindow(this);
		if (bim == null)
			return;
		
		fr = new ArrayList<FloodRegion>();  //Empty arraylist of floodregions
		height = bim.getHeight();
		width = bim.getWidth();
		selectionStart = new Point(0, 0);
		selectionEnd = new Point(width - 1, height - 1);
		sliderTol = 102.0;
		filteredbim = new BufferedImageSP(bim.getBim());  //reset filteredbim to base image
	...
	public void endSelection() {
		fr = new ArrayList<FloodRegion>();  //Empty arraylist of floodregions
		selecting = false;
		fixSelection();  //fix selection to coordinates lowest first, highest second
		filterImage();  //run basic image filtering
		repaint();
	}
	...
	public void filterImage() {
		if (bim == null)
			return;
		filteredbim.edgeDetect();  //perform edge detection on image using 0 sum sharpen mask (edge detect)
		filteredbim.blackAndWhite(30); //threshold image
		filteredFlag = true;
		mapBounds();  //perform general image segmentation and object catagorization
		showFinal();  //processed image set to be render display
		this.repaint();
	}

	// Enforce that start location is always less than end location for both x and y
	// coordinates
	...
	public boolean inSelection(int startX, int startY) {
		if (startX >= selectionStart.x && startX <= selectionEnd.x && startY >= selectionStart.y
				&& startY <= selectionEnd.y)
			return true;
		return false;
	}

	// Run through each pixel in edge detect image, flood filling regions, regions
	// visited tracked in toFillArray to prevent revisiting already catagorized
	// pixels
			...
			for (int col = 0; col < width; col++) { // for each column
				...
				if (toFillArray[pixel] == 0) { // If current pixel has not been inspected
					...
					if (p1.r == 0 && p1.g == 0 && p1.b == 0) { // If pixel is white
						...
						expand(row, col, toFillArray, rgbData, new pixel(), 0.0); // run flood fill operation starting
																					// at this pixel
	...
	private void assignRegion(int startX, int startY) {
		...
		int max = 10;
		int min = 10000;
		if (pixelsSelected * max < height * width && pixelsSelected * min > height * width) { // if flood region is not
																								// too big or too small
			resultVal = 1; // New value will be 1, meaning considered text and wont be picked up during
							// next run of this method
			finalBounds(startX, startY); // Evaluate this area on the real image (not the edgeDetect)
		} else if (pixelsSelected * min <= height * width)
			resultVal = 5; // Region considered too small, not currently used for anything, but can be if
							// needed in the future
		else
			resultVal = 4; // region too big, considered the background

		// All pixels marked as valid(3) for this flood region will be tagged based on
		// what category above they fell into
	...
	private void expandHelper(int pixelIndex, int[] thisArray, int[] this_rgbData, double tolerance, pixel p2,
			pixel p3) {
	...
	//Build final image composition by overlaying processed FloodRegions over the appropriate anchor points
	private BufferedImage buildComp() {
		...
		for (int it = 0; it < fr.size(); it++) { //for each floodregion
			...
			int i_dest, i_source;
			//write over matching submatrix of original image
	...
	//Grabs x coordinate from all floodRegions to find which is at the left most position
	...
	//Generate a matching letter to the one issued by argument, and compare the fitness function to the leftmost letter in original image
	public String genLetter(String letter, Boolean toast) {
		String fonts[] = new String[] { "Arial", "Georgia", "Rockwell Extra Bold", "Comic Sans MS", "Arial Narrow" };
		// String fonts[] =
		// GraphicsEnvironment.getLocalGraphicsEnvironment().getAvailableFontFamilyNames();  <---Future definition for fonts once fontSize disparity is resolved
		int target = leftMostFloodRegion();
		Point dims = fr.get(target).getDim();
		int bestGuess = 0;  //best guess set at first font
		int bestScore = -10000;  //best initial score set unreasonably low
		for (int font_iterator = 0; font_iterator < fonts.length; font_iterator++) {  //for each font
			BufferedImageSP letterbim = new BufferedImageSP(dims.x, dims.y);  //build a new bufferedimageSP
			letterbim.renderLetter(letter, fonts[font_iterator], fr.get(target).getPad());  //render target letter to the image
			int[] fillArrayGen = letterbim.buildFillArray();  //create the array containing values for location of text
			int[] fillArrayReal = fr.get(target).getFillArray();  //grab the array computed for the floodregion
			int matchScore = 0;
			for (int i = 0; i < dims.x * dims.y; i++) {  //create fitness function score
				matchScore = matchScore + fillArrayGen[i] * fillArrayReal[i];
			}
			if (matchScore > bestScore) {  //update best info if current font is better than previous best
				bestScore = matchScore;
				bestGuess = font_iterator;
			}
		}
		if(toast)
			JOptionPane.showMessageDialog(null, fonts[bestGuess]); //pop-up window with results
		return fonts[bestGuess];
	}

	//configure fonts name, size and color based on gathering selected options and variable assignments
	private void processText(Graphics2D big) {
		if (filteredFlag && textBool && fr.size() > 0) { // Text rendering rules
			big.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_GASP);
	...
	public void paintComponent(Graphics g) {
		...
		if (selecting) { // as long as both points are defined
			...
			big.drawRect(x, y, width, height); // draw a red rectangle around the area the user is right click and drag
												// selecting
		}
		processText(big);
